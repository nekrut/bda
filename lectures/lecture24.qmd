...
![wfmash two-step alignment of two sequences. The green diagonal shows colinear mapping segments identified by MashMap (approximate mapping via locality-sensitive hashing). The gray shapes are wavefront expansions from the WFA algorithm performing base-level alignment within each segment. Small gray shapes = high similarity (WFA stays near the diagonal); large triangular shapes = indels or divergent regions forcing WFA to explore more search space. Source: [PGGB GitHub repository](https://github.com/pangenome/pggb).](../images/lecture24/wfmash.png){width=50%}
...
The raw graph from seqwish can contain local complexity---underalignment artifacts where truly homologous positions are split across multiple nodes. `smoothxg` resolves this:

1. Identifies collinear blocks of paths through the graph
2. Applies **Partial Order Alignment (POA)** within each block
3. Iterates multiple passes to handle boundary effects

![smoothxg workflow: identifying blocks, applying POA, and iterating. Source: [PGGB GitHub repository](https://github.com/pangenome/pggb).](../images/lecture24/smoothxg_workflow.png){width=80%}

![Example of graph smoothing: before and after smoothxg. Source: [PGGB GitHub repository](https://github.com/pangenome/pggb).](../images/lecture24/smoothxg.png){width=50%}
