...
The repository now lives at **`galaxyproject/loom`**; in this lecture you build it from source off the default branch.
...
::: {.callout-warning}
## Your prompt should end in `$`, not `#`

If your prompt looks like `root@DESKTOP-XXX:/#` (ends in `#`), you are logged in as **root** --- this is wrong, and `npm` will refuse to install in this state.

It usually means you skipped the Ubuntu first-run setup (closed the window during the username prompt, never rebooted, or typed `wsl` in PowerShell before ever opening the Ubuntu app). To fix it, **inside the root shell**:

```bash
adduser yourname              # pick any lowercase username
...
Close every Ubuntu window, reopen the **Ubuntu** app from the Start menu --- the prompt should now end in `$` and show `yourname@...`.
:::
...
# Clone Loom into your Linux home directory (NOT /mnt/c/)
cd ~
git clone https://github.com/galaxyproject/loom.git
cd loom
